...
public interface ProviderRespository extends JpaRepository<Provider, Long> {
	...
	@Query("SELECT p FROM Provider p WHERE (:search is null OR p.name like CONCAT('%', :search, '%')OR p.email like CONCAT('%', :search, '%') ) AND (:professionId = 0 OR p.profession.id = :professionId)")
	List<Provider> findByProfessionAndEmailAndSearch(int professionId, String search, PageRequest pageRequest);
